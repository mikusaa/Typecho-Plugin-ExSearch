...
- `/ExSearch?action=api&key=...`：公开只读，返回轻量索引 JSON。索引包含标题、链接、日期、分类、标签等元数据，不包含正文全文。
- `/ExSearch?action=query&key=...&q=...`：公开只读，按关键词查询文章与页面正文，最多返回 20 条结果与摘要片段。
- `/ExSearch?action=rebuild`：需要后台登录，用于重建索引。

开启静态化后，插件目录下的 `cache/cache-*.json` 只保存轻量索引；正文全文搜索仍通过 `query` 接口按需完成。
...
如果站点内容很多，建议开启静态化。当前版本的静态缓存只保存轻量索引，不再保存完整正文，因此通常不需要调大数据库 `max_allowed_packet`。
